  ...
  return (
    <div className="mb-6">
      ...
      <div className="flex gap-5">
        {data.map((item) =>
          item.logo_path ? (
            <div key={`logo-${item.id}`} className="bg-white py-1 px-2 rounded-md">
              <img
                src={baseImgUrl + item.logo_path}
                alt={item.name}  // Görsel erişilebilirliği için alt metni ekledim
            ...
            <span
              ...
              key={`name-${item.id}`}  // Benzersiz bir key için `name-` öneki ekledim
            >
              {item.name}
            </span>
          )
